...
if ($("#example").length > 0) {
  $(document).ready(function () {
    ...
    // REALTIME
    pb.collection("dooms").subscribe("*", function () {
    ...
    $("#confirmDeleteBtn").on("click", async function () {
      ...
      for (const id of idsToDelete) {
        try {
          await pb.collection("dooms").delete(id);
        } catch (err) {
...
if ($("#doomForm").length > 0) {
  $(document).ready(function () {
    $("#doomForm").on("submit", async function (e) {
      ...
      try {
        await pb.collection("dooms").create(data);
...
if (recordId && $("#doomEditForm").length > 0) {
  ...
  async function loaddoomData() {
    try {
      const record = await pb.collection("dooms").getOne(recordId);

      $("#doom_id").val(record.id);
  ...
  $("#doomEditForm").on("submit", async function (e) {
    e.preventDefault();
    ...
    try {
      await pb.collection("dooms").update(recordId, data);
